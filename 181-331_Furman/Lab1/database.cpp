...
void DataBase::connectToDataBase()
{
    ...
    if(!QFile("D:/YandexDisk/Study/Qt/Furman_semester2/181-331_Furman/Lab1" DATABASE_NAME).exists()){
...
bool DataBase::openDataBase()
{
    ...
    db.setDatabaseName("D:/YandexDisk/Study/Qt/Furman_semester2/181-331_Furman/Lab1" DATABASE_NAME);
    if(db.open()){
        return true;
    } else {
        return false;
...
bool DataBase::inserIntoTable(const QVariantList &data)
{
    ...
    QSqlQuery query;
    /* В начале SQL запрос формируется с ключами,которые потом связываются методом bindValue 
